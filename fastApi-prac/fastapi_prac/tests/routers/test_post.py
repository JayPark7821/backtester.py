...
async def test_create_post(
    ...
    assert {"id": 1, "body": "Test Post", "user_id": registered_user["id"]}.items() <= response.json().items()
...
async def test_get_all_posts_sorting_likes(
    ...
    response = await async_client.get("/post", params={"sorting": "most_like"})
    assert response.status_code == 200
